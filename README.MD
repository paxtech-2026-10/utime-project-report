...
- **JavaScript / TypeScript (Angular):** se siguen las guías oficiales de Angular Style Guide y Airbnb JavaScript Style Guide. Uso de `camelCase` para variables y funciones, `PascalCase` para clases y componentes, e `kebab-case` para nombres de archivos.
...
**Frontend Web (Angular):** build de producción (`ng build --configuration production`) y publicación del artefacto resultante en GitHub Pages o servicio equivalente.

**Backend (Spring Boot en Azure):** despliegue como **Azure App Service** con base de datos gestionada **Azure Database for MySQL**. Las variables de entorno (cadena de conexión, secretos JWT, claves externas) se configuran en el panel **Configuration → Application settings** del App Service.
...
Para el Sprint #1, centrado en la landing page estática, el equipo aplicó pruebas manuales de diseño responsive y validación de formularios sobre Chrome y Firefox (desktop + mobile). La suite formal de pruebas automatizadas (Cucumber + Gherkin) se incorporará a partir del Sprint #2 con la integración del frontend Angular y el backend Spring Boot.
